...
import { fetchAdminById, updateAdmin } from '@jobify/appwrite-server/collections/admin-collection';
...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const dbAdmin = await fetchAdminById(id);
        if (!dbAdmin) {
            throw new Error('You are not authorized to perform this action');
        }
        const u = dbAdmin as Record<string, unknown>;
