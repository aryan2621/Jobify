...
import { AlertCircle, FileText, Info, XIcon } from 'lucide-react';
import { Badge, BadgeProps } from '@jobify/ui/badge';
import { ApplicationStatus } from '@jobify/domain/application';
import { Clock, CheckCircle, XCircle } from 'lucide-react';
import { Card, CardContent, CardFooter, CardHeader, CardTitle } from '@jobify/ui/card';
import { Button } from '@jobify/ui/button';
import { Job } from '@jobify/domain/job';
import { formatDate } from '@/lib/job-utils/utils';
import { memo } from 'react';
import { Tooltip, TooltipTrigger, TooltipContent } from '@jobify/ui/tooltip';
import { TooltipProvider } from '@jobify/ui/tooltip';
import { Progress } from '@jobify/ui/progress';
import { FORM_STEPS } from '@/app/applications/_lib/utils';
...
export const StatusBadge = ({ status, showText = true, customText, ...props }: StatusBadgeProps) => {
    ...
    return (<Badge variant={variant} className='flex items-center' {...props}>
            {icon}
            {text}
        </Badge>);
};
...
export const CoverLetterDisplay = ({ coverLetter, emptyStateVariant = 'icon', className = '' }: CoverLetterDisplayProps) => {
    if (coverLetter) {
        return (<div className={`whitespace-pre-line ${className}`}>
...
export const ApplicationTimeline = ({ status }: {
    status: ApplicationStatus;
}) => {
    ...
    return (<div className='pt-4 pb-2'>
            <div className='flex items-center justify-between'>
                <div className={`flex flex-col items-center ${getStepStatus(ApplicationStatus.APPLIED, status) === 'completed'
            ? 'text-primary'
            ...
            : getStepStatus(ApplicationStatus.APPLIED, status) === 'current'
                ...
                : 'border border-muted-foreground'}`}>
                        {getStepStatus(ApplicationStatus.APPLIED, status) === 'completed' ? (<CheckCircle className='h-4 w-4'/>) : (<Clock className='h-4 w-4'/>)}
                    </div>
                    <span className='text-xs'>Applied</span>
                </div>

                <div className={`w-full mx-4 h-1 ${getStepStatus(ApplicationStatus.APPLIED, status) === 'completed' ? 'bg-primary' : 'bg-muted'}`}></div>

                <div className={`flex flex-col items-center ${getStepStatus(ApplicationStatus.SELECTED, status) === 'completed'
            ? 'text-primary'
            ...
            : getStepStatus(ApplicationStatus.SELECTED, status) === 'current'
                ? 'border-2 border-primary'
                : 'border border-muted-foreground'}`}>
                        {getStepStatus(ApplicationStatus.SELECTED, status) === 'completed' ? (<CheckCircle className='h-4 w-4'/>) : (<CheckCircle className='h-4 w-4'/>)}
                    </div>
                    <span className='text-xs'>Selected</span>
                </div>

                <div className={`w-full mx-4 h-1 ${getStepStatus(ApplicationStatus.REJECTED, status) === 'completed' || status === ApplicationStatus.REJECTED
            ? 'bg-primary'
            : 'bg-muted'}`}></div>

                <div className={`flex flex-col items-center ${getStepStatus(ApplicationStatus.REJECTED, status) === 'completed'
            ? 'text-primary'
            ...
            : getStepStatus(ApplicationStatus.REJECTED, status) === 'current'
                ? 'border-2 border-primary'
                : 'border border-muted-foreground'}`}>
                        {getStepStatus(ApplicationStatus.REJECTED, status) === 'completed' ? (<CheckCircle className='h-4 w-4'/>) : (<XCircle className='h-4 w-4'/>)}
                    </div>
    ...
    return (<Card>
            ...
            <CardContent className='space-y-4'>
                ...
                <div>
                    ...
                    <div className='flex flex-wrap gap-2 mt-2'>
                        {job.skills.map((skill, index) => (<Badge key={index} variant='outline'>
                                {skill}
                            </Badge>))}
                    </div>
                </div>
            ...
            <CardFooter className='bg-muted/20 pt-3'>
                <Button variant='outline' className='w-full' asChild>
                    <Link href={`/jobs?jobId=${job.id}`}>View Full Job Details</Link>
                </Button>
            </CardFooter>
        </Card>);
};
export const ProgressStepper = memo(({ currentStep, totalSteps, onStepClick }: {
    currentStep: number;
    totalSteps: number[];
    ...
    return (<div className='mb-8'>
                <div className='flex justify-between mb-2'>
                    {FORM_STEPS.map((step, index) => (<TooltipProvider key={`step-${index}`}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <button onClick={() => onStepClick(step.id)} className={`flex flex-col items-center transition-colors ${currentStep >= step.id ? 'text-primary' : 'text-muted-foreground'}`} aria-current={currentStep === step.id ? 'step' : undefined}>
                                        <div className={`
...
export const FormSectionTitle = memo(({ title, subtitle, icon }: {
    title: string;
    subtitle?: string;
...
export const FormField = memo(({ label, error, touched, children, required = false, helpText, }: {
    label: string;
    error?: string;
    touched?: boolean;
    ...
    helpText?: string;
            ...
            {children}
            {touched && error && (<div className='text-destructive text-sm mt-1 flex items-center'>
                    <AlertCircle className='w-3.5 h-3.5 mr-1.5'/>
                    {error}
                </div>)}
        </div>));
FormField.displayName = 'FormField';
export const RemoveButton = memo(({ onClick, label = 'Remove' }: {
    onClick: () => void;
    label?: string;
