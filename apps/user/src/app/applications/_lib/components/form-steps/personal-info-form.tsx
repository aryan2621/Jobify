...
import { Application, JobSource } from '@jobify/domain/application';
import { memo } from 'react';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@jobify/ui/select';
import { AtSign, MapPin, Phone } from 'lucide-react';
import { Gender } from '@jobify/domain/application';
            ...
            <FormField label='Phone Number' error={validation.phone?.errorMessage} touched={validation.phone?.touched} required>
                <div className='flex gap-2'>
                    ...
                    <div className='relative flex-grow'>
                        ...
                        <Input id='phone' placeholder='Phone Number' value={formData.phone} onChange={(e) => onFieldChange('phone', e.target.value)} className={`pl-10 ${validation.phone?.touched && !validation.phone?.isValid ? 'border-destructive' : ''}`}/>
                    </div>
                </div>
            </FormField>

            <FormField label='Current Location' error={validation.currentLocation?.errorMessage} touched={validation.currentLocation?.touched} required>
                <div className='relative'>
                    <div className='absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none'>
                        <MapPin className='h-4 w-4 text-muted-foreground'/>
                    </div>
                    <Input id='current-location' placeholder='City, Country' value={formData.currentLocation} onChange={(e) => onFieldChange('currentLocation', e.target.value)} className={`pl-10 ${validation.currentLocation?.touched && !validation.currentLocation?.isValid ? 'border-destructive' : ''}`}/>
                </div>
            </FormField>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                <FormField label='Gender' required>
                    <Select value={formData.gender} onValueChange={(value) => onFieldChange('gender', value)}>
                        ...
                        <SelectContent>
                            {Object.values(Gender).map((gender) => (<SelectItem key={gender} value={gender}>
                                    {gender}
                                </SelectItem>))}
                        </SelectContent>
                    </Select>
                ...
                <FormField label='How did you hear about us?' required>
                    <Select value={formData.source} onValueChange={(value) => onFieldChange('source', value as JobSource)}>
                        ...
                        <SelectContent>
                            {Object.values(JobSource).map((source) => (<SelectItem key={source} value={source}>
                                    {source}
                                </SelectItem>))}
                        </SelectContent>
                    </Select>
