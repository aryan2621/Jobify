...
import { User as UserIcon, MapPin, CheckCircle, Download, ArrowLeft, FileText, Inbox, Lightbulb, AlertCircle, Mail, Link as LinkIcon, } from 'lucide-react';
...
export default function UserApplicationDetailPage({ params }: {
    ...
    return (<NavbarLayout>
        <div className='container mx-auto px-4 py-6'>
            ...
            </Alert>) : loading ? (<div className='space-y-4'>
            ...
            </div>) : application && job ? (<div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                <div className='space-y-4'>
                    <Card>
                        ...
                        <CardContent className='space-y-4'>
                            ...
                            {application.status === ApplicationStatus.SELECTED && (<Alert>
                            ...
                            {application.status === ApplicationStatus.REJECTED && (<Alert>
                                <AlertCircle className='h-4 w-4' />
                        ...
                        <CardFooter className='bg-muted/20 pt-3'>
                            <Button onClick={() => downloadResume(application.resume)} disabled={fetchingResume || !application.resume} className='w-full'>
                        ...
                        <CardContent>
                            <Tabs value={activeTab} onValueChange={setActiveTab}>
                                ...
                                <TabsContent value='details'>
                                    ...
                                    <div className='space-y-2 mb-6'>
                                        ...
                                        <div className='flex flex-wrap gap-2'>
                                            ...
                                            {application.skills.length === 0 && (<p className='text-sm text-muted-foreground'>No skills provided</p>)}
                                        </div>
                                    </div>

                                    {application.socialLinks.length > 0 && (<div className='space-y-2'>
                                        <h3 className='font-medium flex items-center'>
                                            <LinkIcon className='h-4 w-4 text-primary mr-2' />
                                            Social Links
                                        </h3>
                                        <div className='space-y-1'>
                                            {application.socialLinks.filter(Boolean).map((link, index) => (<a key={index} href={link.startsWith('http') ? link : `https://${link}`} target='_blank' rel='noopener noreferrer' className='block text-primary hover:underline text-sm'>
                                                {link}
                                            </a>))}
                                        </div>
                                    </div>)}
                                ...
                                <TabsContent value='experience'>
                                    {application.experience.length > 0 ? (<div className='space-y-4'>
                                        {application.experience.map((exp, index) => (<Card key={index} className='bg-muted/30'>
                                            <CardContent className='p-4'>
                                                <div className='flex justify-between items-start'>
                                                    <div>
                                                        <h3 className='font-medium'>{exp.profile}</h3>
                                                        <p className='text-sm'>{exp.company}</p>
                                                        {exp.employer && exp.employer !== exp.company && (<p className='text-xs text-muted-foreground'>Employer: {exp.employer}</p>)}
                                                    </div>
                                ...
                                <TabsContent value='education'>
                                    {application.education.length > 0 ? (<div className='space-y-4'>
                                        {application.education.map((edu, index) => (<Card key={index} className='bg-muted/30'>
                                            <CardContent className='p-4'>
                                                <div className='flex justify-between items-start'>
                                                    <div>
                                ...
                                <TabsContent value='coverLetter'>
                                    <Card className='bg-muted/30'>
                                        <CardContent className='p-4'>
                                            {application.coverLetter ? (<div className='whitespace-pre-line'>
