...
import { Clock, FileText, Link as LinkIcon, Upload, CheckCircle, ArrowLeft, ExternalLink, AlertCircle, Info } from 'lucide-react';
...
export default function AssignmentSubmissionPage() {
    ...
    const isFormValid = submissionData.repoLink.trim() !== '';
    useEffect(() => {
        const loadAssignment = async () => {
            if (!applicationId || !nodeId) return;
            setIsLoadingAssignment(true);
            try {
                const details = await ky
                    .get(`/api/application?id=${encodeURIComponent(String(applicationId))}&nodeId=${encodeURIComponent(String(nodeId))}`)
                    .json<AssignmentDetails>();
                setAssignment(details);
            } catch (error) {
                toast({
                    title: 'Failed to load assignment',
                    description: 'Please retry from the assignment email link.',
                    variant: 'destructive',
                });
            } finally {
    ...
    if (isSubmitted) {
        return (<div className="flex flex-col items-center justify-center min-h-[70vh] p-6 text-center">
                ...
                <p className="text-muted-foreground text-lg mb-8 max-w-md">
                    Your submission for <b>{assignment?.label ?? 'Assignment'}</b> has been received. Our team will review it and get back to you soon.
                </p>
    ...
    if (isLoadingAssignment) {
        return <div className="container mx-auto max-w-5xl py-12 px-4 sm:px-6">Loading assignment...</div>;
    }
    if (!assignment) {
        return <div className="container mx-auto max-w-5xl py-12 px-4 sm:px-6">Assignment not found.</div>;
    }
                ...
                <div className="lg:col-span-12">
                    <div className="mb-6 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        <div>
                            ...
                            <h1 className="text-4xl font-extrabold tracking-tight">Assignment: {assignment.label}</h1>
                        </div>
                    ...
                    <Card className="border-none shadow-sm bg-muted/30">
                        ...
                        <CardContent className="space-y-6">
                            <div className="prose dark:prose-invert max-w-none text-foreground/80 leading-relaxed">
                                {assignment.description || 'Please complete and submit this assignment.'}
                            </div>
                        ...
                        <CardContent className="p-6 pt-8 space-y-6">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                {!isFormValid && (<div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 text-xs bg-amber-50 dark:bg-amber-950/20 p-3 rounded-lg border border-amber-100 dark:border-amber-900/30">
                                        ...
                                        Please provide a repository link to proceed.
                                    </div>)}
