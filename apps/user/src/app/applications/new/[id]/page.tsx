...
export default function JobApplicationForm({ params }: {
    ...
    const validateForm = useCallback(() => {
        ...
        formData.experience.forEach((exp, index) => {
            ...
            if (!profileValid || !companyValid || !startDateValid)
                isValid = false;
        });
    ...
    const handleFiles = useCallback(async (files: FileList | null) => {
        ...
        try {
            ...
            const response = (await ky.post('/api/upload-resume', { body: formDataToSend }).json()) as {
                fileId: string;
            };
    ...
    const handleUpdateSocialLink = useCallback((index: number, value: string) => {
        ...
        return (<NavbarLayout>
                <div className='flex items-center justify-center h-[calc(100vh-4rem)]'>
                    <div className='flex flex-col items-center space-y-4'>
                        ...
                        <Button onClick={() => router.push('/jobs')}>Go back to job listings</Button>
                    </div>
                </div>
            </NavbarLayout>);
    }
                    ...
                    : `Step ${page} of ${FORM_STEPS.length}: ${FORM_STEPS[page - 1].title}`}
                                        ...
                                        <CardContent>
                                            <form id='application-form' onSubmit={handleSubmit}>
                                                {!showPreviewMode ? (<>
                                                        <ProgressStepper currentStep={page} totalSteps={FORM_STEPS.map((s) => s.id)} onStepClick={goToPage}/>
                                                        ...
                                                        <AnimatePresence mode='wait'>
                                                            <motion.div key={`page-${page}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                                                                ...
                                                                {page === 2 && (<div className='space-y-6'>
                                                                        ...
                                                                        {formData.education.map((edu, index) => (<EducationForm key={`edu-${index}`} education={edu} index={index} formData={formData} validation={validation} setFormData={setFormData} onFieldChange={handleFieldChange}/>))}
                            ...
                            setFormData({
                                ...
                                education: [
                                    ...
                                    new Education('', '', DegreeType.BACHELOR, 0),
                                                                ...
                                                                {page === 3 && (<div className='space-y-6'>
                                                                        <FormSectionTitle title='Experience' subtitle='Add your work experience' icon={<Briefcase className='w-5 h-5 text-primary'/>}/>

                                                                        {formData.experience.map((exp, index) => (<ExperienceForm key={`exp-${index}`} experience={exp} index={index} formData={formData} validation={validation} setFormData={setFormData} onFieldChange={handleFieldChange}/>))}
                        ...
                        setPage(validationErrors.length > 0
                            ? Math.min(...Object.keys(validation)
                            ...
                            : 1);
                                                    ...
                                                    <Button type='submit' form='application-form' disabled={submitting || validationErrors.length > 0} className='relative'>
                                                        {submitting ? (<>
                                                    ...
                                                    {page < FORM_STEPS.length ? (<Button type='button' onClick={goToNextPage}>
                                                            Continue
