...
export default function ProfilePersonalTab({ profile, setProfile, submitting, updateUser }: ProfilePersonalTabProps) {
    ...
    return (<form onSubmit={handleUpdateName} className='space-y-4'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                <div>
                    ...
                    <Input id='firstName' name='firstName' value={profile.firstName} onChange={(e) => setProfile({ ...profile, firstName: e.target.value })}/>
                </div>
                <div>
                    <Label htmlFor='lastName'>Last Name</Label>
                    <Input id='lastName' name='lastName' value={profile.lastName} onChange={(e) => setProfile({ ...profile, lastName: e.target.value })}/>
                </div>
            </div>
