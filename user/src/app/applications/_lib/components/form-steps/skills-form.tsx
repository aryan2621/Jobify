...
export const SkillsForm = memo(({ formData, validation, predefinedSkills, onFieldChange, onAddSocialLink, onRemoveSocialLink, onUpdateSocialLink, }: {
    ...
    return (<div className='space-y-8'>
                <div>
                    <FormSectionTitle title='Skills' subtitle='Select your relevant skills for this position' icon={<Lightbulb className='w-5 h-5 text-primary'/>}/>

                    <FormField label='Skills' error={validation.skills?.errorMessage} touched={validation.skills?.touched} required>
                        <div className='space-y-4'>
                            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2'>
                                {predefinedSkills.map((skill, index) => (<label key={index} className={`
                ...
                onFieldChange('skills', newSkills);
                            ...
                            <div className='flex items-end gap-2'>
                                <div className='flex-grow'>
                                    ...
                                    <Input id='new-skill' value={newSkill} onChange={(e) => setNewSkill(e.target.value)} placeholder='Enter a custom skill'/>
                                </div>
                                <Button onClick={handleAddSkill} type='button' disabled={!newSkill.trim() || formData.skills.includes(newSkill.trim())}>
                                    Add Skill
                                </Button>
                            </div>

                            <div className='flex flex-wrap gap-2 mt-2'>
                                {formData.skills.map((skill, index) => (<Badge key={index} className='py-1.5 px-2'>
                                        {skill}
                                        <button type='button' className='ml-2 text-xs hover:text-accent-foreground' onClick={() => {
                ...
                <div>
                    <FormSectionTitle title='Social Links' subtitle='Add your professional profiles and websites' icon={<Link className='w-5 h-5 text-primary'/>}/>
            ...
            if (files && files.length > 0) {
                onFieldChange('resume', '', -1);
                    ...
                    <div className='space-y-4 mt-6'>
                        {formData.socialLinks.map((link, index) => (<div key={`social-link-${index}`} className='flex items-center gap-2'>
                                <div className='flex-grow'>
                                    <Input value={link} onChange={(e) => onUpdateSocialLink(index, e.target.value)} placeholder='https://' className={validation[`socialLink_${index}`]?.touched && !validation[`socialLink_${index}`]?.isValid
                ? 'border-destructive'
                : ''}/>
                                    {validation[`socialLink_${index}`]?.touched && !validation[`socialLink_${index}`]?.isValid && (<p className='text-destructive text-xs mt-1'>{validation[`socialLink_${index}`]?.errorMessage}</p>)}
                                </div>
