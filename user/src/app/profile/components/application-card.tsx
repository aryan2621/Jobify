...
const ApplicationCard = ({ application }: ApplicationCardProps) => {
    ...
    const currentJobTitle = application.experience && application.experience.length > 0 ? application.experience[0].profile : 'No previous experience';
    ...
    return (<Card className='mb-4 hover:shadow-md transition-shadow'>
            <CardContent className='p-5'>
                ...
                {application.skills && application.skills.length > 0 && (<div className='mt-3 pt-3 border-t'>
                        <div className='flex flex-wrap gap-1.5'>
                            {application.skills.slice(0, 5).map((skill, index) => (<Badge key={index} variant='outline' className='text-xs'>
                                    {skill}
                                </Badge>))}
                            {application.skills.length > 5 && (<Badge variant='outline' className='text-xs'>
