...
function createUserCollection() {
    database
        ...
        return Promise.all([
            ...
            database.createStringAttribute(DB_NAME, USER_COLLECTION, 'avatarFileId', 100, false),
        ]);
    })
        .catch((error) => {
...
async function createUserDocument(user: User) {
    try {
        ...
        return await database.createDocument(DB_NAME, USER_COLLECTION, user.id, {
            ...
            ...(user.avatarFileId && { avatarFileId: user.avatarFileId }),
        });
    }
    catch (error) {
...
export async function ensureAvatarFileIdAttribute(): Promise<void> {
    try {
        await database.createStringAttribute(DB_NAME, USER_COLLECTION, 'avatarFileId', 100, false);
    }
    catch (err: unknown) {
        const msg = (err as {
            message?: string;
...
async function removeJobFromUser(userId: string, jobId: string) {
    try {
        const user = await fetchUserByUserId(userId);
        const jobs = (user.jobs ?? []).filter((id: string) => id !== jobId);
        return await database.updateDocument(DB_NAME, USER_COLLECTION, userId, { jobs });
    }
    catch (error) {
        console.log('Error removing job from user', error);
        throw error;
    }
}
export { createUserCollection, createUserDocument, fetchUserByEmail, fetchUserByUsername, fetchUserByUserId, setApplicationToUser, setJobToUser, removeJobFromUser, updateUser, addWorkflowToUser, deleteWorkflowFromUser, };
