...
export const fetchPaginatedJobs = async (limit: number, lastId: string | null, onSuccess: (jobs: Job[], hasMore: boolean, lastId: string | null) => void, onError: () => void) => {
    try {
...
export const deleteJob = async (jobId: string) => {
    try {
        await ky.delete(`/api/post?id=${encodeURIComponent(jobId)}`);
        toast({
            title: 'Job Deleted',
