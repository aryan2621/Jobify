...
const FilterRow: React.FC<FilterRowProps> = ({ filter, updateFilter, removeFilter }) => (<div className='flex flex-col space-y-2 w-full mb-4'>
        <div className='flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full'>
            <Select value={filter.field || ''} onValueChange={(value) => updateFilter({ ...filter, field: value })}>
                ...
                <SelectContent>
                    {fields.map((field) => (<SelectItem key={field} value={field}>
                            {field}
                        </SelectItem>))}
                </SelectContent>
            </Select>
            <Select value={filter.operator || ''} onValueChange={(value) => updateFilter({ ...filter, operator: value })}>
                ...
                <SelectContent>
                    {operators.map((op) => (<SelectItem key={op} value={op}>
                            {op}
                        </SelectItem>))}
                </SelectContent>
            </Select>
            {filter.operator !== 'exist' && filter.operator !== 'not exist' && (<Input type='text' placeholder='Value' value={filter.value || ''} onChange={(e) => updateFilter({ ...filter, value: e.target.value })} className='col-3'/>)}
            <TooltipProvider>
                <Tooltip>
                    <TooltipTrigger asChild>
                        <Button variant='ghost' size='icon' onClick={() => removeFilter(filter.id)} className='p-0 h-8 w-8 flex items-center justify-center col-1'>
...
interface FilterGroupProps {
    ...
    updateFilter: (groupId: number, filter: {
        id: number;
        field: string;
        operator: string;
        value: string;
...
const FilterGroup: React.FC<FilterGroupProps> = ({ group, updateFilter, removeFilter, addFilter, updateGroupOperator }) => {
    ...
    return (<div key={group.id} className='space-y-4 mb-6 w-full'>
            {group.filters.map((filter, filterIndex) => (<React.Fragment key={filter.id}>
                    <FilterRow filter={filter} updateFilter={(updatedFilter) => updateFilter(group.id, updatedFilter)} removeFilter={() => removeFilter(group.id, filter.id)}/>
                    {filterIndex < group.filters.length - 1 && (<div className='flex justify-center my-2'>
...
export default function FiltersPage() {
    ...
    const updateFilter = (groupId: number, updatedFilter: {
        id: number;
        field: string;
        operator: string;
        value: string;
    ...
    return (<>
            <Card className='w-full mx-auto'>
                ...
                <CardContent>
                    {filterGroups.map((group) => (<FilterGroup key={group.id} group={group} updateFilter={updateFilter} removeFilter={removeFilter} addFilter={addFilter} updateGroupOperator={updateGroupOperator}/>))}
