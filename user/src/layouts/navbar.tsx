...
import { CircleHelpIcon, FileIcon, MenuIcon, ProfileIcon } from '@/components/elements/icon';
import { BarChart, BookCopyIcon, HouseIcon, ScrollTextIcon, PanelLeftClose, PanelLeftOpen, HandCoins, LogOut } from 'lucide-react';
...
const NavbarLayout: React.FC<NavbarLayoutProps> = ({ children }) => {
    ...
    if (!mounted)
        return null;
    return (<div className='flex min-h-screen w-full bg-background/95'>
            <motion.aside className='hidden md:flex flex-col border-r bg-background/80 backdrop-blur-sm shadow-sm h-screen sticky top-0' initial={isCollapsed ? 'collapsed' : 'expanded'} animate={isCollapsed ? 'collapsed' : 'expanded'} variants={sidebarVariants} transition={{ duration: 0.2 }}>
                <div className='flex items-center h-14 px-3 border-b'>
                    {!isCollapsed && (<motion.div initial='expanded' animate={isCollapsed ? 'collapsed' : 'expanded'} variants={labelVariants} className='font-semibold text-lg'>
                            JobBoard
                        </motion.div>)}
                ...
                {user ? (<div className='flex flex-col h-full justify-between py-2'>
                        <div>
                            ...
                            <nav className='flex flex-col gap-1 px-2'>
                                <TooltipProvider delayDuration={isCollapsed ? 300 : 10000}>
                                    ...
                                    <NavItem href='/contact' icon={<CircleHelpIcon className='h-5 w-5'/>} label='Help and Support' isCollapsed={isCollapsed} isActive={pathname === '/contact'}/>
                                </TooltipProvider>
                            </nav>
                        </div>
                        ...
                        <div className='mt-auto px-2'>
                            <TooltipProvider delayDuration={isCollapsed ? 300 : 10000}>
                                <NavItem href='/profile' icon={<ProfileIcon className='h-5 w-5'/>} label='Profile' isCollapsed={isCollapsed} isActive={pathname === '/profile'}/>
                            </TooltipProvider>
                        </div>
            ...
            <div className='flex flex-1 flex-col'>
                            ...
                            {user ? (<div className='flex flex-col h-full'>
                                    ...
                                    <div className='flex-1 overflow-auto py-2'>
                                        <nav className='grid gap-1 px-2'>
                                            ...
                                            <NavItem href='/contact' icon={<CircleHelpIcon className='h-5 w-5'/>} label='Help and Support' isCollapsed={false} isActive={pathname === '/contact'}/>
                                        </nav>
                                    </div>

                                    <div className='mt-auto border-t p-2'>
                                        <NavItem href='/profile' icon={<ProfileIcon className='h-5 w-5'/>} label='Profile' isCollapsed={false} isActive={pathname === '/profile'}/>
                                    </div>
                                </div>) : (<div className='flex items-center justify-center h-full'>
                                    <p className='text-muted-foreground text-sm'>Please log in</p>
                                </div>)}
                ...
                <main className='min-h-0 p-4 sm:p-6 overflow-auto'>{children}</main>
            </div>
        </div>);
};
...
const NavItem: React.FC<NavItemProps> = ({ href, icon, label, isCollapsed, isActive = false, badge, variant = 'default' }) => {
    ...
    const iconElement = (<>
            {icon}
            {badge && !isCollapsed && (<Badge variant='secondary' className='ml-auto text-xs'>
                    {badge}
                </Badge>)}
        </>);
    return isCollapsed ? (<Tooltip>
        ...
        </Tooltip>) : (<Link href={href} className={`${baseClass} ${variantClass} ${activeClass} ${alignClass}`} prefetch={false}>
            {iconElement}
            <span>{label}</span>
            {badge && (<Badge variant='secondary' className='ml-auto text-xs'>
                    {badge}
                </Badge>)}
        </Link>);
};
