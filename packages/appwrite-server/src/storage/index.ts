...
function getAvatarViewUrl(fileId: string): string {
    ...
    return `${endpoint}/v1/storage/buckets/${STORAGE}/files/${fileId}/view?project=${project}`;
}
...
async function uploadAvatar(file: File, userId: string): Promise<string> {
    ...
    try {
        ...
        try {
            await storage.createFile(STORAGE, fileId, file, permissions);
        }
        catch (err: unknown) {
            const code = (err as {
                code?: number;
...
async function getResume(fileId: string) {
    try {
        return await storage.getFileDownload(STORAGE, fileId);
    }
    catch (error) {
